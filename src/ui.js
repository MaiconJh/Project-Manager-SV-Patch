// @context: export-preset-file save-dialog open-dialog apply-rehydrate feedback ui-diagnostics preset-ui-fix safe-bind tree-scroll-preserve files-header-sticky breadcrumb kb-nav hit-test bind-guard bind-singularity fatal-hooks.
/*
PM-SV-PATCH META
...
if (!window.__uiFatalHooksInstalled) {
  window.__uiFatalHooksInstalled = true;
  window.addEventListener("error", (e) => {
    console.error("[UI FATAL]", e?.message, e?.error);
  });
  window.addEventListener("unhandledrejection", (e) => {
    console.error("[UI FATAL PROMISE]", e?.reason);
  });
}
...
function _bindClickSafe(id, handler, opts = {}) {
  ...
  node.addEventListener("click", (ev) => {
    Promise.resolve()
      .then(() => handler(ev))
      .catch((e) => {
        console.error("[UI] Handler failed:", id, e);
...
function _uiHitTestAtCenter(node) {
  if (!node || typeof node.getBoundingClientRect !== "function") return null;
  const r = node.getBoundingClientRect();
  const cx = Math.max(0, Math.min(window.innerWidth - 1, Math.floor(r.left + Math.max(1, r.width) / 2)));
  const cy = Math.max(0, Math.min(window.innerHeight - 1, Math.floor(r.top + Math.max(1, r.height) / 2)));
  const top = document.elementFromPoint(cx, cy);
  return { cx, cy, top };
}

function _diagWhyNotClickable(node, id) {
  if (!__PM_UI_DEV__) return;
  if (!node) {
    console.error(`[UI][HitTest] ${id}: missing node`);
    return;
  }
  const hit = _uiHitTestAtCenter(node);
  const top = hit?.top || null;
  const ncs = getComputedStyle(node);
  const parent = node.closest(".panelHead, .panelTitleRow, .presetQuick") || node.parentElement;
  const pcs = parent ? getComputedStyle(parent) : null;
  const tcs = top ? getComputedStyle(top) : null;
  const blocked = top && top !== node && !node.contains(top);
  if (!blocked) {
    console.info(`[UI][HitTest] ${id}: OK`);
    return;
  }
  console.warn(`[UI][HitTest] ${id}: BLOCKED by ${top?.id ? '#'+top.id : top?.tagName}`, {
    node: { pointerEvents: ncs.pointerEvents, zIndex: ncs.zIndex, visibility: ncs.visibility, display: ncs.display, opacity: ncs.opacity },
    parent: parent ? { pointerEvents: pcs.pointerEvents, zIndex: pcs.zIndex, visibility: pcs.visibility, display: pcs.display, opacity: pcs.opacity } : null,
    top: tcs ? { pointerEvents: tcs.pointerEvents, zIndex: tcs.zIndex, visibility: tcs.visibility, display: tcs.display, opacity: tcs.opacity } : null,
    point: { x: hit?.cx, y: hit?.cy },
  });
}

function _runUiGlobalHitTestReport() {
  if (!__PM_UI_DEV__) return;
  if (process?.env?.PM_UI_DIAG_HITTEST !== "1") return;
  const ids = ["btnOpen", "btnRefreshFiles", "btnExportPrimary", "btnPresetMenu", "presetMenuSave", "presetMenuImport"];
  for (const id of ids) _diagWhyNotClickable(el(id), id);
}
...
function _reportUiBindHealth() {
  ...
  try {
    ...
    console.info("[UI] bind complete:", { wired: wiredCount, missing: missingCount });
...
function _bindPresetMenuUi() {
  ...
  _bindClickSafe("presetMenuSave", () => {
    _setPresetMenuOpen(false);
    el("btnPresetSaveFile")?.click();
  }, { required: false });
  _bindClickSafe("presetMenuImport", () => {
    _setPresetMenuOpen(false);
...
function _buildFileRow(r, snapshot) {
  ...
  const rowPath = _normalizeAbsPath(r.absPath);
  const isSelectedRow = r.id === selectedId || (selectedAbsPath && rowPath === selectedAbsPath);
  row.className = "row" + (isSelectedRow ? " selected" : "") + (r.ignored ? " ignored" : "") + (picked ? " row-picked" : "");
  row.dataset.absPath = rowPath;
  row.tabIndex = 0;
...
try{
  ...
  _withPreservedTreeScroll(() => {
    ...
    if (!snapshot.tree) {
      ...
      _updateFilesBreadcrumb(snapshot, "");
      return;
    }
...
function _getTreeScrollContainer() {
  return el("fileList");
}

function _captureTreeScrollState(preferredAnchorPath = "") {
  const list = _getTreeScrollContainer();
  if (!list) return null;
  const rowH = Math.max(20, Number(_fileListVirtual.rowHeight || 40));
  const scrollTop = Math.max(0, Number(list.scrollTop || 0));
  const approxIdx = Math.floor(scrollTop / rowH);
  const offset = scrollTop - approxIdx * rowH;
  const fallbackPath = (_fileRowsCache && _fileRowsCache[approxIdx]) ? _normalizeAbsPath(_fileRowsCache[approxIdx].absPath) : "";
  const anchorPath = _normalizeAbsPath(preferredAnchorPath || fallbackPath || "");
  const state = {
    scrollTop,
  ...
  if (__PM_UI_DEV__) console.debug("[TREE_SCROLL] capture", state);
  return state;
}

function _restoreTreeScrollState(state) {
  if (!state) return;
  const list = _getTreeScrollContainer();
  if (!list) return;
  const rows = Array.isArray(_fileRowsCache) ? _fileRowsCache : [];
  let targetTop = Math.max(0, Number(state.scrollTop || 0));
  if (state.anchorPath) {
    const idx = rows.findIndex((r) => _normalizeAbsPath(r.absPath) === state.anchorPath);
    if (idx >= 0) targetTop = Math.max(0, idx * state.rowH + Number(state.anchorOffset || 0));
  }
  const maxTop = Math.max(0, Number((list.scrollHeight || 0) - (list.clientHeight || 0)));
  const clamped = Math.max(0, Math.min(maxTop, targetTop));
  list.scrollTop = clamped;
  if (__PM_UI_DEV__) console.debug("[TREE_SCROLL] restore", { wanted: targetTop, applied: clamped, maxTop, anchorPath: state.anchorPath });
}

function _withPreservedTreeScroll(work, preferredAnchorPath = "") {
  const captured = _captureTreeScrollState(preferredAnchorPath);
  const result = typeof work === "function" ? work() : undefined;
  requestAnimationFrame(() => _restoreTreeScrollState(captured));
  return result;
}

function _formatBreadcrumbPath(snapshot, absPath) {
  const full = String(absPath || "").trim();
  if (!full) return { text: "(nothing selected)", title: "" };
  const root = _normalizeAbsPath(snapshot?.projectPath || "");
  const normalized = _normalizeAbsPath(full);
  let display = normalized;
  if (root && normalized.startsWith(root)) {
    display = normalized.slice(root.length).replace(/^\/+/, "");
  }
  const parts = display ? display.split("/").filter(Boolean) : [];
  const shown = parts.length > 4 ? [parts[0], "…", parts[parts.length - 2] || "", parts[parts.length - 1] || ""] : parts;
  const text = shown.length ? ["root", ...shown].join(" / ") : "root";
  return { text, title: normalized };
}

function _updateFilesBreadcrumb(snapshot, preferredAbsPath = "") {
  const elBc = el("filesBreadcrumb");
  if (!elBc) return;
  const rows = Array.isArray(_fileRowsCache) ? _fileRowsCache : [];
  let abs = _normalizeAbsPath(preferredAbsPath || selectedAbsPath || "");
  if (abs && !rows.some((r) => _normalizeAbsPath(r.absPath) === abs)) {
    abs = "";
  }
  if (!abs && selectedId != null) {
    const hit = rows.find((r) => r.id === selectedId);
    if (hit?.absPath) abs = _normalizeAbsPath(hit.absPath);
  }
  if (!abs) {
    elBc.textContent = "(nothing selected)";
    elBc.title = "";
    return;
  }
  selectedAbsPath = abs;
  const b = _formatBreadcrumbPath(snapshot, abs);
  elBc.textContent = b.text;
  elBc.title = b.title;
}

function _setupKeyboardFocusModality() {
  const onKey = (ev) => {
    if (["Tab", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "Home", "End", "PageUp", "PageDown"].includes(ev.key)) {
      document.body.classList.add("kbNav");
    }
  };
  const onPointer = () => document.body.classList.remove("kbNav");
...
function bind() {
  if (window.__uiBindRan) {
    console.warn("[UI] bind already executed, skipping duplicate");
    return;
  }
  window.__uiBindRan = true;
  _loadExportUiConfig();
  try{
...
// bootstrap
function _runUiBootstrapOnce() {
  bind();
  ipcRenderer.invoke("ui:refresh");
}
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", _runUiBootstrapOnce, { once: true });
} else {
  _runUiBootstrapOnce();
}
